    ...
    return (
        ...
        <nav className="flex items-center justify-between p-4 lg:px-8" >
            ...
            <div className="hidden items-center md:flex lg:gap-x-10 md:gap-x-4">
                {links ? links.map((link, e) => (<Link key={e} to={link.to} className="text-sm font-semibold leading-6 text-gray-900 transition hover:text-indigo-600">{link.title}</Link>))
                    : null}
                ...
                <div className="mt-6 flow-root">
                    <div className="-my-6 divide-y divide-gray-500/10">
                        <div className="space-y-2 py-6">
                            {links ? links.map((link, i) => (<Link key={i} to={link.to} onClick={() => setMobileMenuOpen(false)} className="-mx-3 block rounded-lg px-3 py-2 text-base text-center font-semibold leading-7 text-gray-900 hover:bg-indigo-300">{link.title}</Link>))
                                : null}
                        </div>
                    </div>
                </div>
